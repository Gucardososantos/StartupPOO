...
class Config {
    public int totalRodadas(){ return 3; }
...
class ConsoleApp {
    ...
    private final String[] opcoes = {"MARKETING", "EQUIPE", "PRODUTO", "INVESTIDORES", "CORTARCUSTOS"};
    ...
    public void start() {
        ...
        System.out.println("Deseja jogar com BOT automático? (s/n): ");
        boolean modoBot = sc.nextLine().trim().equalsIgnoreCase("s");
        Random random = new Random(42); // Seedada para resultados sempre iguais
        ...
        try (PrintWriter pw = new PrintWriter("relatorio_ranking.csv", StandardCharsets.UTF_8)) {
            ...
            pw.println(s.getNome()+","+
                    ...
                    s.getMoral().valor()+","+
                    score);
            System.out.println("Ranking exportado para relatorio_ranking.csv!");
        } catch(Exception e){
